

using System.Linq.Expressions;

string Testinput = "11-22,95-115,998-1012,1188511880-1188511890,222220-222224,1698522-1698528,446443-446449,38593856-38593862,565653-565659,824824821-824824827,2121212118-2121212124";
int Testresult = 1227775554;
string PuzzleInput = "8284583-8497825,7171599589-7171806875,726-1031,109709-251143,1039-2064,650391-673817,674522-857785,53851-79525,8874170-8908147,4197684-4326484,22095-51217,92761-107689,23127451-23279882,4145708930-4145757240,375283-509798,585093-612147,7921-11457,899998-1044449,3-19,35-64,244-657,5514-7852,9292905274-9292965269,287261640-287314275,70-129,86249864-86269107,5441357-5687039,2493-5147,93835572-94041507,277109-336732,74668271-74836119,616692-643777,521461-548256,3131219357-3131417388";

long result = 0;
List<List<long>> ListOfRanges = new List<List<long>>();


List<string> Inputlist = PuzzleInput.Split(',').ToList();
foreach (string s in Inputlist)
{
    List<string> StartAndEndOfRange = s.Split('-').ToList();
    long start = long.Parse(StartAndEndOfRange[0]);
    long end = long.Parse(StartAndEndOfRange[1]);
    List<long> Range = new List<long>();
    
    for (long i = start; i < end+1 ; i++)
        {
            Range.Add(i); 
        }
    ListOfRanges.Add(Range);
};


foreach (List<long> Range in ListOfRanges)
{
    foreach (long ID in Range)
    {
        int Increment = 1;
        string IDString = ID.ToString();
        while (Increment <= IDString.Length / 2)
        {
            if (ID.ToString().Length % Increment == 0)
            {
                string Block= ID.ToString().Substring(0,Increment);
                string RepeatedBlock = String.Concat(Enumerable.Repeat(Block, IDString.Length / Increment ));
                if (RepeatedBlock == IDString)               
                {
                    result += ID;
                    break;
                }
            }
            Increment++;
        }
    }
};

System.Console.WriteLine(result);
Console.ReadKey();

